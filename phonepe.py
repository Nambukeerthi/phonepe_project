...
if select == "HOME":
    ...
    with tab1:
        ...
        if method_1 == "Aggrecated insurance": 
            ...
            states_name.sort()
            fig_india_1 = px.choropleth(tacyg_test, geojason = data1, locations= "States", featureidkey = "properties.ST_NM", color = " Transaction_amount", color_continues_scale = "rainbow", range_color = tacyg_test["Transaction_amount "].min(),tacyg
               _test["Transaction_amount "].max()), hover_name = "States", title = f"{year} "TRANSACTION AMOUNT",fitbounds ="locations",height = 600, width = 600)    
