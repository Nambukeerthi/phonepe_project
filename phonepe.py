...
if select == "HOME":
    ...
    with tab1:
        ...
        if method_1 == "Aggrecated insurance":
            ...
            fig_amount = px.bar(tacyg_test, x = "States", y = "Transaction_amount", tittle = "TRANSACTION AMOUNT", color_discrete_sequence = px.colors.sequential.bluered_r)
            # fig_amount.show()
            st.plotly_chart(fig_amount, theme=None, use_container_width=True)   
            fig_amount = px.bar(tacyg_test, x = "States", y = "Transaction_count", tittle = "TRANSACTION COUNT", color_discrete_sequence = px.colors.sequential.aggrnyl)
            st.plotly_chart(fig_amount, theme=None, use_container_width=True)
